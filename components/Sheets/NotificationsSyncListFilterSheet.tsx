...
const OS_ARRAY = [
  ...
  },
  {
    ...
    label: 'iOS',
    icon: <ExpoImage
      source={require('@/assets/images/apple600.png')}
      style={{ width: 16, height: 16 }}
    />,
  },
  {
    ...
    label: 'Windows',
    icon: <ExpoImage
      source={require('@/assets/images/windows600.png')}
      style={{ width: 16, height: 16 }}
    />,
  },
  {
    ...
    label: 'Linux',
    icon: <ExpoImage
      source={require('@/assets/images/linux600.png')}
      style={{ width: 16, height: 16 }}
    />,
  },
  {
        ...
        >
          <YStack gap='$4'>
            ...
            <YStack gap='$2'>
              ...
              <XStack gap='$3' flexWrap='wrap'>
                {(devices || []).filter((f) => f.os === 'android').map((device) => (
                  <Button
                    key={device.id}
                    size={'$3'}
                    ...
                    onPress={() => {
                      if (localFilters.tailscaleId.includes(device.id)) {
                        setLocalFilters((prev) => ({ ...prev, tailscaleId: prev.tailscaleId.filter((id) => id !== device.id) }));
                      } else {
                        setLocalFilters((prev) => ({ ...prev, tailscaleId: [...prev.tailscaleId, device.id] }));
                      }
                    }}
                  >
                    <Text>
                      {device?.name?.split('.')[0] || device.id}
                    </Text>
                  </Button>
                ))}
              </XStack>
